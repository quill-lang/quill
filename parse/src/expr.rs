    ...
    I: Iterator<Item = TokenTree>,
{
    ...
    fn parse_forall_binder(&mut self, indent: usize) -> Dr<PTypeBinder, ParseError> {
        match self.next() {
            ...
            Some(tt) => Dr::new_err(todo!()),
            None => todo!(),
        }
    }

    /// Assuming that the next token is a `forall`, parse a `forall <types>, e` type.
    fn parse_forall_type(&mut self, min_indent: usize, indent: usize) -> Dr<PType, ParseError> {
    ...
    pub fn parse_type(&mut self, indent: usize) -> Dr<PType, ParseError> {
        if matches!(
            ...
            self.parse_forall_type(indent, indent)
        } else {
